...
*   **1. Detección de Ejecución de Comandos Sospechosos:**
    ...
    *   **Configuración de Sysmon:**
        ...
        ```
   *   **Ejemplo:** Como dice [4], podemos cazar comandos sospechosos como `schtasks.exe` (para persistencia) o como menciona Smiliotopoulos et al. [6], `wmic.exe` (para exfiltración).
...
*   **1. Detección de Inyección de Código:**
    ...
    *   **Configuración de Sysmon:**
        ...
        ```
   *   **Ejemplo:** Como dice [4, 5], Mimikatz usa esto para acceder a `lsass.exe` y robar credenciales.
...
*   **2. Detección de Acceso a Procesos Sensibles:**
    ...
    *   **Configuración de Sysmon:**
        ...
        ```
  *  **Ejemplo:** Como dice [1] es importante configurar Sysmon para detectar accesos a procesos sensibles como `lsass.exe`.
...
*   **Mejora del Rendimiento:** Al reducir el volumen de eventos generados, se reduce la carga en el sistema de monitorización y en los sistemas de almacenamiento y análisis de logs (como un SIEM) [1, 9, 11, 12]. Elgh [11] también menciona la necesidad de optimizar el rendimiento de estos sistemas.
...
*   **Falsos Negativos:** Un filtro demasiado restrictivo puede omitir eventos importantes, generando falsos negativos y permitiendo que una amenaza pase desapercibida [4, 11].  [11] también incide en este problema. La necesidad de balancear la granularidad y el detalle de los datos se puede ver reflejado en el artículo de [5].
...
*   **Rendimiento Deficiente:** Un filtro mal optimizado puede impactar negativamente en el rendimiento del sistema [9, 11], tal como señala [11].
...
Este ejemplo ilustra la necesidad de balancear la reducción de ruido y la visibilidad sobre actividades maliciosas en la configuración de los filtros. El diseño óptimo dependerá del entorno específico, tal como remarcan las investigaciones de [4].
