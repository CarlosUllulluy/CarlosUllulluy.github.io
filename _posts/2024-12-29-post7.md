...
# 1. Empleando Procmon y Sysmon en la Investigación de Procesos Sospechosos

## 1.1. Complementariedad de Procmon y Sysmon

Procmon y Sysmon, aunque funcionales de manera independiente, exhiben una notable complementariedad en la captura de eventos del sistema, lo que proporciona una visión más holística de las actividades dentro de un entorno Windows. Esta sinergia es particularmente valiosa para la detección de amenazas, como se articula en el trabajo de Mavroeidis y Jøsang [4] y se sustenta en la investigación sobre la detección de malware avanzado de Mahmoud et al. [5], donde se destaca el uso combinado de Sysmon con la ELK stack. De manera análoga, el estudio de Smiliotopoulos et al. [6] sobre la detección de movimientos laterales con Sysmon subraya la utilidad de estas herramientas en la identificación de comportamientos anómalos.

*   **Procmon (Process Monitor):** Como se describe en el informe de Blumira [1], Procmon es una herramienta de monitorización en tiempo real que expone la actividad del sistema de archivos, el Registro y los procesos/hilos. Integrado en la suite Sysinternals, ofrece una perspectiva granular de las operaciones a nivel de sistema operativo, incluyendo la creación de procesos, el acceso a archivos y al registro, las conexiones de red y la carga de DLLs [3, 12]. Su principal fortaleza radica en la captura exhaustiva de eventos en tiempo real y su capacidad de filtrado no destructivo, como se indica en la documentación oficial de Microsoft [12]. Además, como señalan Orton et al. [2], el uso de herramientas como Procmon es fundamental para comprender el comportamiento detallado de los procesos en un sistema.
*   **Sysmon (System Monitor):** Según la documentación oficial de Microsoft [3, 12] y lo expuesto en el estudio de Orton et al. [2], Sysmon es un servicio de sistema y un controlador de dispositivos que registra la actividad del sistema en el registro de eventos de Windows. Se enfoca en eventos críticos para la seguridad, como la creación de procesos (incluyendo la línea de comandos completa y los hashes), conexiones de red, modificaciones en los tiempos de creación de archivos y cambios en el registro. Sysmon enriquece estos eventos con metadatos significativos, como el GUID del proceso, el hash y la relación con el proceso padre [4, 12]. Permite, además, la configuración de reglas para la detección de patrones específicos de actividad maliciosa, tal como se menciona en el trabajo de Mavroeidis y Jøsang [4]. Igualmente, es destacable su capacidad para rastrear cambios en el registro y la creación de flujos alternativos de datos (ADS), según el trabajo de Smiliotopoulos et al. [6].

**Análisis de la Complementariedad:**

*   **Amplitud vs. Profundidad:** Procmon ofrece una visión amplia y detallada de las actividades del sistema en tiempo real [1], mientras que Sysmon proporciona una perspectiva más enfocada en la seguridad, con eventos filtrados y configurables [3, 12]. Este punto se ve reforzado por la investigación de Elgh [11], donde se enfatiza la necesidad de filtrar eventos para un análisis eficaz. No obstante, el análisis conjunto de los datos obtenidos por ambas herramientas puede aumentar considerablemente las capacidades de detección, como se observa en los estudios de [4, 5, 6].
...
En suma, Procmon habilita una inspección meticulosa en tiempo real, mientras que Sysmon facilita la monitorización continua y la detección de amenazas a largo plazo [1, 3, 4, 12]. La combinación de ambas herramientas resulta en una capacidad de análisis forense robusta, como se evidencia en la investigación de Mahmoud et al. [5]. Además, la flexibilidad de Sysmon permite la adaptación a necesidades específicas del entorno, y como sugieren Xie et al. [9] la optimización de la recolección de logs es fundamental para reducir el impacto en el sistema. En este sentido Sysmon aporta flexibilidad en el uso de filtros [1, 3, 12].

## 1.2. Detección de Comportamiento Malicioso: Un Caso Práctico
...
La combinación de Procmon y Sysmon permite detectar la inyección de DLLs al observar la carga de la DLL sospechosa en tiempo real con Procmon y correlacionarla con los eventos de Sysmon que registran la creación del hilo remoto y la carga de la imagen. La información adicional proporcionada por Sysmon, como el proceso que origina la inyección, facilita la investigación y la respuesta al incidente. La efectividad del sistema de detección se ve reflejada en el trabajo de Smiliotopoulos et al. [6] mediante un análisis similar para la detección de movimientos laterales.
...
*   **1. Detección de Ejecución de Comandos Sospechosos:**
    ...
    *   **Configuración de Sysmon:**
        ...
        ```
  *   **Ejemplo:** Como dice Mavroeidis et al. [4], podemos cazar comandos sospechosos como `schtasks.exe` (para persistencia) o como menciona Smiliotopoulos et al. [6], `wmic.exe` (para exfiltración).
...
*   **1. Detección de Inyección de Código:**
    ...
    *   **Configuración de Sysmon:**
        ...
        ```
   *   **Ejemplo:** Como dice el trabajo de Mavroeidis et al. [4] y Mahmoud et al. [5], Mimikatz usa esto para acceder a `lsass.exe` y robar credenciales.
...
*   **2. Detección de Acceso a Procesos Sensibles:**
    ...
    *   **Configuración de Sysmon:**
        ...
        ```
   *  **Ejemplo:** Como dice Blumira [1], podemos usar Sysmon para detectar accesos a la memoria de procesos, especialmente a `lsass.exe`.
...
Los Alternate Data Streams (ADS) son una característica del sistema de archivos NTFS de Windows que permite asociar metadatos o flujos de datos adicionales a un archivo sin modificar su contenido principal [2]. Estos flujos son "invisibles" para la mayoría de las aplicaciones y herramientas de Windows, incluyendo el Explorador de Windows y el comando `DIR`, tal como señalan Smiliotopoulos et al. [6].
...
*   **Ocultación de datos:** Permiten ocultar código malicioso, herramientas o datos robados en un archivo aparentemente inofensivo [2], como se demuestra en la investigación de Smiliotopoulos et al. [6]. Elgh [11] también confirma esta práctica como un método de evasión.
...
*   El filtro `condition="contains">:\$DATA` es importante para enfocar la búsqueda en los ADS.
*   La exclusión de `explorer.exe` reduce el ruido ya que crea ADS legítimamente al descargar archivos de internet, tal como apunta la investigación de Smiliotopoulos et al. [6].
...
*   **Reducción de Ruido:** Los filtros permiten enfocarse en eventos relevantes para la detección de amenazas, eliminando el ruido generado por actividades legítimas [4, 11, 12]. Esta capacidad es fundamental para un análisis eficiente, como señalan Mahmoud et al. [5]. Adicionalmente, la tesis de Elgh [11] destaca la necesidad de filtrar eventos para un análisis eficaz y Xie et al. [9] remarcan la importancia de optimizar la recolección de logs mediante muestreo aleatorio.
*   **Mejora del Rendimiento:** Al reducir el volumen de eventos generados, se reduce la carga en el sistema de monitoreo y en los sistemas de almacenamiento y análisis de logs (como un SIEM) [1, 9, 11, 12]. Elgh [11] también menciona la necesidad de optimizar el rendimiento de estos sistemas.
...
*   **Falsos Negativos:** Un filtro demasiado restrictivo puede omitir eventos importantes, generando falsos negativos y permitiendo que una amenaza pase desapercibida [4, 11]. Elgh [11] tambien incide en este problema. La necesidad de balancear la granularidad y el detalle de los datos se puede ver reflejado en el artículo de Mahmoud et al. [5].
...
*   **Rendimiento Deficiente:** Un filtro mal optimizado puede impactar negativamente en el rendimiento del sistema [9, 11], tal como señala Elgh [11].

Un diseño deficiente de filtros compromete la eficacia en la detección de amenazas, incrementa el consumo de recursos y conduce a la pérdida de información valiosa para la investigación de incidentes. Por consiguiente, un diseño adecuado, tal y como remarcan Russinovich et al. [12], es de vital importancia.
...
Este ejemplo ilustra la necesidad de balancear la reducción de ruido y la visibilidad sobre actividades maliciosas en la configuración de los filtros. El diseño óptimo dependerá del entorno específico, tal como remarcan las investigaciones de Mavroeidis et al. [4].
...
[6] Smiliotopoulos, C.; Barmpatsalou, K.; Kambourakis, G. Revisiting the Detection of Lateral Movement through Sysmon. *Appl. Sci.* **2022**, *12*, 7746.
[7] Elgh, J. (2022). *Comparison of adversary emulation tools for reproducing behavior in cyber attacks* (Master's thesis). Linköping University.
